...
const recentData = [
  { id: 1, icon: <GrDocumentText size={24} />, Title: "Product Strategy 2024", editedAt: "Edited 4 hours ago ", md: "1.3MB" },
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-row justify-between items-center px-4">
            ...
            <div className="flex flex-row items-center gap-1 p-1 rounded bg-secondary/20">
              <button
                className={`p-1 rounded ${viewMode === "grid" ? "bg-white text-primary" : "text-gray-400"}`}
                onClick={() => setViewMode("grid")}
              >
                <RxDashboard size={15} />
              </button>
              <button
                className={`p-1 rounded ${viewMode === "list" ? "bg-white text-primary" : "text-gray-400"}`}
...
const FavoriteCards = ({ title, text, icon, viewMode }: { title: string, text: string, icon: React.ReactNode, viewMode: "grid" | "list" }) => {
  if (viewMode === "list") {
    return (
      <div className="rounded-4xl bg-white border gap-4 p-4 flex flex-row items-center justify-between hover:bg-gray-50 transition-colors">
